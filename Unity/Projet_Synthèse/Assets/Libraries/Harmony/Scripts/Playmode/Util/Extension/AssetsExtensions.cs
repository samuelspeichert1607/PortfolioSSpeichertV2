...
namespace Harmony
{
    ...
    public static class AssetsExtensions
    {
        ...
        public static IList<T> FindAssets<T>() where T : Object
        {
            ...
            foreach (string assetGuid in UnityEditor.AssetDatabase.FindAssets(string.Format("t:{0}", typeof(T))))
            {
                T asset = UnityEditor.AssetDatabase.LoadAssetAtPath<T>(UnityEditor.AssetDatabase.GUIDToAssetPath(assetGuid));
                if (asset != null)
                {
                    assets.Add(asset);
                }
        ...
        {
            string[] scenesPath = Directory.GetFiles(Path.GetFullPath(Application.dataPath), name + ".unity", SearchOption.AllDirectories);

            if (scenesPath.Length == 1)
            {
                return scenesPath[0].Replace(Path.GetFullPath(Path.Combine(Application.dataPath, "..")) + "\\", "");
            }
        ...
        {
            string[] scenesPath = Directory.GetFiles(Path.GetFullPath(Path.Combine(Application.dataPath, folderPath)),
                                                     "*.unity",
