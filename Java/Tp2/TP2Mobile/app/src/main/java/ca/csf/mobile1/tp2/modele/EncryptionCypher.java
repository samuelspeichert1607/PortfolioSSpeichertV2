...
/**
 * Cette classe sert, en tant que modèle, à crypter un message.
 */
public class EncryptionCypher {
    ...
    /** Cette fonction servira à crypter le message recu en paramètre.
     * @param inputMessage Le message que l'on désire crypter
     ...
     */
    public String encryptMessage(String inputMessage, char[] validInputCharacters, char[] validOutputCharacters){
        ...
        if(inputMessage != null) {
            for (int i = 0; i < inputMessage.length(); i++) {
                for (int j = 0; j < validInputCharacters.length - 1; j++) {
                    if (inputMessage.charAt(i) == validInputCharacters[j]) {
                        outputMessage += validOutputCharacters[j];
                    }
                }
            }
        }
